...
EVRInitError CHmdDriver::Activate(TrackedDeviceIndex_t unObjectId)
{
...
void CHmdDriver::Deactivate()
{
    m_unObjectId = vr::k_unTrackedDeviceIndexInvalid;
}

void CHmdDriver::EnterStandby()
{
}

void* CHmdDriver::GetComponent(const char* pchComponentNameAndVersion)
{
...
void CHmdDriver::PowerOff()
{
}

void CHmdDriver::DebugRequest(const char* pchRequest, char* pchResponseBuffer, uint32_t unResponseBufferSize)
{
...
void CHmdDriver::GetWindowBounds(int32_t* pnX, int32_t* pnY, uint32_t* pnWidth, uint32_t* pnHeight)
{
    *pnX = m_nWindowX;
    *pnY = m_nWindowY;
...
bool CHmdDriver::IsDisplayOnDesktop()
{
    return true;
}

bool CHmdDriver::IsDisplayRealDisplay()
{
    return false;
}

void CHmdDriver::GetRecommendedRenderTargetSize(uint32_t* pnWidth, uint32_t* pnHeight)
{
    *pnWidth = m_nRenderWidth;
    *pnHeight = m_nRenderHeight;
}

void CHmdDriver::GetEyeOutputViewport(EVREye eEye, uint32_t* pnX, uint32_t* pnY, uint32_t* pnWidth, uint32_t* pnHeight)
{
    *pnY = 0;
...
void CHmdDriver::GetProjectionRaw(EVREye eEye, float* pfLeft, float* pfRight, float* pfTop, float* pfBottom)
{
    *pfLeft = -1.0;
    *pfRight = 1.0;
    *pfTop = -1.0;
...
DistortionCoordinates_t CHmdDriver::ComputeDistortion(EVREye eEye, float fU, float fV)
{
...
vr::DriverPose_t CHmdDriver::GetPose()
{
    ...
    if ((GetAsyncKeyState(VK_UP) & 0x8000) != 0) {
        pZ += -0.01 * cos(yaw) * cos(pitch);
        pY += 0.01 * sin(pitch);
        pX += -0.01 * sin(yaw) * cos(pitch);
    }
    ...
    if ((GetAsyncKeyState(VK_LEFT) & 0x8000) != 0) {
        pZ += -0.01 * cos(yaw + 1.57) * cos(pitch);
        pX += -0.01 * sin(yaw + 1.57) * cos(pitch);
    }
